...
export default function AdminBusinessesPage() {
  ...
  const approve = useMutation({
    mutationFn: (id: string) => approveBusinessDeactivation(id),
    onSuccess: () => {
  ...
  const reject = useMutation({
    mutationFn: (id: string) => rejectBusinessDeactivation(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["businesses"] });
                  ...
                  <tr key={biz.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        >
                          {approve.isPending && approve.variables === biz.id ? (
                        ...
                        >
                          {reject.isPending && reject.variables === biz.id ? (
                            <span className="w-3 h-3 border border-white/40 border-t-white rounded-full animate-spin" />
                          ) : (
                            <XCircle size={13} />
                          )}
